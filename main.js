...
const loadBtn=document.querySelector('.content__btn__wrapper');
...
if (c) {
  const currentList = document.querySelector(`.main-header__categories[value="${c}"]`);
  if (currentActive) {
    currentActive.classList.remove('active');
  }
     ...
     data.forEach(news => {
        ...
        <a href="single.html?id=${news.id}&countryId=${currentCategory}">
            <article class="content__article"  id="article-${news.id}">
                    <img src="${news.yoast_head_json.og_image[0].url}" alt="" class="content__img" style="width: 500px; max-width: 100%; height: 300px; object-fit: cover;>
                    <h4 class="content__title">${news.title.rendered}</h4>
                    <p class="content__content">${news.excerpt.rendered}</p>
                    <h6 class="content__publisheddate">${news.yoast_head_json.article_published_time.substring(0,10)}</h6>
            </article>
        </a>
        `
...
const fetchNews=async function(country_id=c || albId){
    try{
            ...
            let data=await fetch(`${API_BASE_URL}?per_page=10&categories=${country_id}`);
...
const fetchMoreNews=async function(){
    try{
        ...
        let data=await fetch(`${API_BASE_URL}?per_page=10&page=${currentPage}&categories=${currentCategory}`);
        data=await data.json();
        renderNews(data);
